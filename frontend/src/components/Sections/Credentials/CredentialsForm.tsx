...
function CredentialsForm({type,setCredentialsData, submitForm, credentialsData}:CredentialsFormProps) {
    return (
            <form onSubmit={submitForm} className='flex_center flex-col gap-4 mt-4'>
                ...
                <input type="text" id="email" name="email" className='input_1' placeholder='Enter Your Email/UserName/Phone' value={credentialsData.usernameOrEmailOrPhone} onChange={(e)=>setCredentialsData({...credentialsData,usernameOrEmailOrPhone: e.target.value})} />
