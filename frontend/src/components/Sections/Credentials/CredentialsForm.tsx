...
function CredentialsForm({type,setCredentialsData, submitForm, credentialsData, uploadingCredential}:CredentialsFormProps) {
    return (
            <form onSubmit={submitForm} className='flex_center flex-col gap-4 mt-4'>
                <NormalInput type="text" id="websiteUrl" name="websiteUrl" className='input_1' placeholder='Enter Website URL' value={credentialsData.websiteurl} onChange={(e)=>setCredentialsData({...credentialsData,websiteurl: e.target.value})} copy={type === "update"} required/>
